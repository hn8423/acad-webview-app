...
<div class="admin-profile">
	<h1 class="admin-profile__title">내 정보 관리</h1>

	{#if user}
		<form
			class="admin-profile__form"
			onsubmit={(e) => {
				e.preventDefault();
				handleSave();
			}}
		>
			<div class="admin-profile__avatar-section">
				<div class="admin-profile__avatar">
					{#if user.profile_img}
						<img src={user.profile_img} alt={user.user_name} />
					{:else}
						<div class="admin-profile__avatar-placeholder">
			...
			<div class="admin-profile__field" role="group" aria-label="성별">
				<span class="admin-profile__field-label" id="gender-label">성별</span>
				<div class="admin-profile__gender-options" role="radiogroup" aria-labelledby="gender-label">
					<button
						type="button"
						role="radio"
						aria-checked={userGender === 'MALE'}
						class="admin-profile__gender-btn"
						class:admin-profile__gender-btn--active={userGender === 'MALE'}
						onclick={() => {
					...
					<button
						...
						class="admin-profile__gender-btn"
						class:admin-profile__gender-btn--active={userGender === 'FEMALE'}
						onclick={() => {
			...
			<div class="admin-profile__password-section">
				<button
					type="button"
					class="admin-profile__password-toggle"
					onclick={() => {
				...
				>
					<span>비밀번호 변경</span>
					<svg
						class="admin-profile__password-arrow"
						class:admin-profile__password-arrow--open={showPasswordChange}
						width="20"
						height="20"
						viewBox="0 0 20 20"
						fill="none"
				...
				{#if showPasswordChange}
					<div class="admin-profile__password-fields">
						<Input
							type="password"
							label="현재 비밀번호"
			...
			{#if errors['form']}
				<p class="admin-profile__error">{errors['form']}</p>
			{/if}
